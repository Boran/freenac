This directory contains example scripts used when migration a site from V2.0 to V2.2

The v2.0 DB schema was not very normalised, so some data is really difficult to import cleanly, for example the office/location/building fields. These have been centralsied into one lookup table in V2.2.
...
These scripts are based on the fact that  V2.1 uses the db 'opennac' by default, whereas v2.0 uses 'inventory'. This means that both database can co-exist on the same mysql instance.
...
Important note: This script is intended to help you migrate your data. Most of your data should be ok, but please review your data afterwards, because there are some operations that you might need to perform by hand.
