	 ...
	 if (($candidate)&&($argc==1)) //This is an ip which is a good candidate to be scanned and no parameters present on the command line
         {
            if (isset($devices['lastseen'][$l]))
            {
               ...
               $diff=(int)my_date_diff($lastseen,$timestamp,$what_units_time);
...
# PHP 5.3.0 introduced a function called date_diff, which will cause that the script complains saying
# Fatal error: Cannot redeclare date_diff()
# So, the easiest thing to do, is to rename our date_diff function to my_date_diff instead of using their
# date_diff function. For older installations of PHP, date_diff is not available.
function my_date_diff($date1, $date2,$what) //Time difference between the timestamp from the database and the one generated by the script
{
   if ( ! $date1 || ! $date2 || ! $what )
      return false;
