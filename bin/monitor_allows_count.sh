...
#/opt/vmps/logtail /var/log/messages /var/log/.messages.vmps_count | egrep "vmpsd: (ALLOW|DENY)" | awk '{if (($1+0) > 10) print $1 " " $2 " " $3}' | uniq -c | sort -n >> $tempfile2

## extract vmpsd authentication messages per second and count, 
## then sort by number and show lines with more than 8/auths/sec
...
#cat /var/log/messages | egrep "vmpsd: (ALLOW|DENY)" | awk '{print $1 "-" $2 "-" $3}' | uniq -c | awk '{if (($1+0) > 10) print $1 " " $2}' | sort -n

cat /var/log/messages | egrep "vmpsd: (ALLOW|DENY)" | awk '{print $1 "-" $2 "-" $3 " " $4}' | uniq -c | awk '{if (($1+0) > 10) print $1 " " $2 " " $3}' | sort -n     >> $tempfile2
