...
#$count = 100;
$count = 1;
print "Sending $count requests ..\n";
$t0 = new Benchmark;

for ($iter=1;$iter<=$count;$iter++) {

  #print "Sending $iter of $count requests ..\n";
  $buf = makeVQPrequest(%request);
  $buf = sendVQP($request{server_ip},$buf);
  %response = parseVQPresp($buf);
  if ( $response{status} eq "DENY" ) {
    print "DENY  ";
  }
...
print "Vlan: $response{vlan}, MAC Address: $response{macaddr}, Status: $response{status}\n";
